...
bool Sistema::salvarFilaDeProcessos(){
    ...
    for(uint32_t i = 0; i < size; i++){
       ...
       content += std::to_string(temp->getType()) + "|" +
           std::to_string(temp->getPID()) + "|" +
           temp->getInfo() + "\n";
...
void Sistema::createProcess(FIFO<Process*> *processQueue, int type, uint32_t pid, std::string data){
...
bool Sistema::carregarFilaDeProcessos(){
    ...
    std::string content = arquivos.lerArquivo(filename);
